    ...
    def count_neighbors(self, x, y) :
        ...
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                if dx == 0 and dy == 0:
                    continue
                if self.sightline(x,y, dx, dy):
                    count += 1
        return count

    def sightline(self, x, y, dx, dy):
        tx = x
        ty = y
        while True:
            tx += dx
            ty += dy
            if tx < 0 or ty < 0 or tx >= self.height or ty >= self.width:
                return False
            seen = self.board[tx][ty]
            if seen == ".":
                continue
            elif seen == "L":
                return False
...
class Test(unittest.TestCase):
    def test_count(self):
        ...
        self.assertEqual(board.count_neighbors(0,0), 3)
        self.assertEqual(board.count_neighbors(0,2), 1)
        self.assertEqual(board.count_neighbors(3,0), 0)
        self.assertEqual(board.count_neighbors(3,2), 1)
        self.assertEqual(board.count_neighbors(1,1), 2)
